"""
Debug ICICIBANK SHORT Signal at 1:42 PM
"""
...
def main():
    print("=" * 80)
    print("ICICIBANK SHORT SIGNAL ANALYSIS - 1:42 PM")
    print("=" * 80)
    print()
    ...
    print(f"YOUR TRADE:")
    print(f"  SHORT Entry: ₹{entry_price} at 1:42 PM")
    print(f"  Exit (Loss): ₹{exit_price} at 1:49 PM")
    print(f"  Loss: ₹{loss:.2f} ({loss_pct:.2f}%) ❌")
    print(f"  Hold time: 7 minutes")
    print()
    ...
    print(f"ICICIBANK today: {len(df_today)} candles")
    ...
    print("=" * 80)
    print("5-MINUTE DATA (1:30 PM - 2:00 PM) - TODAY's DATA ONLY")
    print("=" * 80)
    print()
    
    try:
        analysis_df = df_today.between_time('13:30', '14:00')
    except:
    ...
    for idx, row in analysis_df.iterrows():
        ...
        highlight = ""
        if "13:40" <= time_str <= "13:45":
            highlight = " <-- ENTRY"
        elif "13:45" <= time_str <= "13:55":
            highlight = " <-- EXIT"
    ...
    print()
    print("=" * 80)
    print("ANALYSIS AT ENTRY TIME (~1:42 PM)")
    print("=" * 80)
    
    try:
        entry_candles = df_today.between_time('13:40', '13:45')
        if len(entry_candles) > 0:
            entry_candle = entry_candles.iloc[0]
            
            print()
            print(f"AT SIGNAL TIME (~13:40):")
            ...
            try:
                ...
                if idx_loc > 0:
                    ...
                    adx_falling = adx_change < -0.5
                    print(f"  ADX Rising: {'✅ YES' if adx_rising else '❌ NO'} (Change: {adx_change:+.1f})")
                    if adx_falling:
                        print(f"  ⚠️ ADX FALLING - trend weakening!")
            except:
                pass
            
            # With NEW rules
            print()
            print("WITH CURRENT STRICT RULES:")
            if st_is_nan:
    ...
    print()
    print("=" * 80)
    print("YOUR 8 TRADES TODAY - SUMMARY")
    print("=" * 80)
    print()
    ...
    print("| 6  | ONGC         | SHORT     | 12:41  | -0.29%  | ✅ Valid |")
    print("| 7  | HCLTECH      | SHORT     | 13:07  | -0.16%  | ✅ Valid |")
    print(f"| 8  | ICICIBANK    | SHORT     | 13:42  | -{loss_pct:.2f}%  | ??? |")
